...
export default function Roomout() {
  ...
  const onRealSubmit = useCallback(
    async (e) => {
      e.preventDefault();
      const btn = document.querySelector('#gombangregister')
      ...
      console.log(roomDealRegisterRequestDto);
      try {
        ...
        // setRoomid(response.data.data.roomDeal.id)
        navigate(`/roomdetail/${response.data.data.roomDeal.id}`)
    ...
    axios
      ...
      .then((response) => {
        ...
        if (documents.length > 0) {
          ...
          axios
            .post(`${process.env.REACT_APP_API_ROOT}/roomdeal/search-nearest`, addressData)
            .then((response) => {
        ...
        <div className={styles.roomoutform}>
          ...
          <div className={styles.gbinfo}>
            ...
            <div className={styles.gbstructure}>
              <label>
                <input
                  ...
                  value="오픈형"
                  checked={struc === "오픈형"}
                  onChange={handleClickStrucButton}
                />
                <p>오픈형</p>
              </label>
              <label>
            ...
            <div className={styles.moneybox}>
              ...
              <div>
                <h3 className={styles.h3_title}>관리비</h3>
                <p className={styles.detailtext}>없을 경우 0을 입력하세요.</p>
                <div className={styles.money}>
                  <input
                    type="number"
                    value={managementFee}
              ...
              <div>
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={totalFloor}
                    onChange={handleTotalFloor}
                    placeholder="건물 전체 층 수"
                  />
                  층
                </div>
              </div>
                ...
                <div className={styles.elevator}>
                  <label>
                    <input
                      type="radio"
                      value="elevatoryes"
                      checked={elevator === "elevatoryes"}
                      onChange={handleClickElevatorButton}
                    />
                    있음
                  </label>
                  <label>
                ...
                <div className={styles.elevator}>
                  <label>
                    <input
                      type="radio"
                      value="parkingyes"
                      checked={parking === "parkingyes"}
                      onChange={handleClickParkingButton}
                    />
                    가능
                  </label>
                  <label>
            ...
            <div className={styles.roomoption}>
              <div>
                ...
                <div>
                  <input type="number" value={pyeong} onChange={handlePyeong} placeholder="평 수" />
                  평
                </div>
              </div>
              <div>
              ...
              <div className={styles.checkboxgroup}>
                {checkBoxList.map((item, idx) => (
                  <div className={styles.checkdiv} key={idx}>
                    ...
                    />
                    <label className={styles.checklabel} htmlFor={item}>
                      {item}
                    </label>
                  </div>
                ))}
            ...
            <div className={styles.textarea}>
              <textarea value={content} onChange={handleContent} rows="5" cols="50" />
            </div>
          </div>
          <div className={styles.check}>
            <input type="checkbox" id="check1" checked={agreement} onChange={agreeBtnEvent} />
            <label htmlFor="check1">
              매물관리규정을 확인하였으며, 입력한 정보는 실제 매물과 다름이 없습니다.{" "}
              <span className="pilsoo">(필수)</span>
            </label>
          </div>
          <button id="gombangregister" onClick={onRealSubmit}>곰방 내놓기</button>
        </div>
      </div>
    </div>
  );
